...
| Grok review | **ADAPTER IN PROGRESS** — requires `XAI_API_KEY` |
| Engine | **paused** |
| Host P | **prohibited** |
...
1. Land programme control plane + Grok adapter PR; add `XAI_API_KEY`.  
2. S1A Gate A then Gate B automatic activation proofs.  
3. S1B → S2 → S3; resolve #74/PR #68; AUTO-1/2/3 Host D E2E proof.  
