...
describe("update-check", () => {
  ...
  */
  ...
  describe("Appsolino managed source", () => {
    ...
    it("performUpdateInstall refuses npm install in managed mode", async () => {
      process.env.FUSION_MANAGED_SOURCE = "appsolino";
      const execFake = vi.fn();

      const result = await performUpdateInstall("0.72.0", "0.73.0", { exec: execFake, fusionDir });

      expect(execFake).not.toHaveBeenCalled();
      expect(result.updated).toBe(false);
      expect(result.error).toContain("Appsolino");
    });

    it("performUpdateCheck returns managed payload without registry fetch", async () => {
      process.env.FUSION_MANAGED_SOURCE = "appsolino";
      process.env.FUSION_MANAGED_SOURCE_STATUS = join(fusionDir, "status.json");
      await writeFile(
        join(fusionDir, "status.json"),
        JSON.stringify({ deployedSha: "deployed-sha", upstreamSha: "upstream-sha", updateStatus: "idle" }),
        "utf-8",
      );
      ...
      const result = await performUpdateCheck(fusionDir, "0.72.0", { releaseRoot: fusionDir });

      expect(fetchSpy).not.toHaveBeenCalled();
      expect(result.managed).toBe(true);
      expect(result.updateAvailable).toBe(false);
      expect(result.latestVersion).toBeNull();
      expect(result.managedStatus).toEqual({
